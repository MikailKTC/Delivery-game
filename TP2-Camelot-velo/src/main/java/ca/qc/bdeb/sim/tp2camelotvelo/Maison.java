...
public class Maison extends ObjetDuJeu {
    ...
    private BoiteAuxLettres creerBoiteAuxLettres() {
        ...
        return new BoiteAuxLettres(new Point2D(position.getX() + decalageBoite, yAleatoire), abonnee);
    }

    private void creerFenetres() {

        final int decalageFenetre = 300;
        final int fenetrePositionY = 50;

        int nbFenetres = (int) (Math.random() * 3); // 0, 1 ou 2

        for (int i = 0; i < nbFenetres; i++) {
            fenetres.add(
                    new Fenetre(new Point2D(position.getX() + decalageFenetre * (i + 1),
                            fenetrePositionY), abonnee)
            );
